...
- **Performance Management** (Super Admin/HR/Manager) — Dashboard/Reports tabs, KPI row (Reviews In Progress/Avg Rating (Org)). **Performance Reviews**: each review carries an editable goal + KPI text, independent **Self-Assessment** and **Manager Assessment** statuses, a 1–5 manager rating, editable **competency/skill-gap notes**, a **Promotion/PIP** flag, and a **360° feedback** note thread anyone can post to. **Rule: a review cannot be marked complete until both self- and manager-assessment are submitted** — the **Mark Review Complete** button is disabled, and the server rejects the request, until both are `Submitted`. Employees get their own **My Reviews** self-service view to submit their self-assessment and read/add 360° feedback. **Reports** tab: ratings by team, rating distribution, status split, Promotion/PIP counts. Every **Key Features** tile is clickable — most jump to the review card that implements it; "Performance Reports & Analytics" switches to the Reports tab.

- **Learning Management** (Super Admin/HR/Manager) — Dashboard/Reports tabs, KPI row (Active Courses/Total Enrolled). **Training Courses**: HR can **+ Add Course** (title, optional pass mark, mandatory flag), **upload PDF/video materials** (stored like employee documents), and **Manage Enrollments** — enroll any active employee, record a score, and toggle completion. **Rule: a certificate is only issued after the associated assessment is passed** — completion alone doesn't certify a course with a pass mark; the employee's score must also meet it (a course with no pass mark certifies on completion). **Only Super Admin** can flip a course's **Allow download/copy** switch; by default materials are **view-only** — employees see them rendered inline (PDF in an embedded viewer, video with the browser's download button hidden) with no download link, only becoming downloadable once Super Admin grants it. This is a UI-level deterrent against casual copying, not cryptographic DRM. Employees get their own **My Courses** self-service view of enrolled courses, scores, certificate status, and protected materials. **Reports** tab: per-course enrolled/completed/certified counts and completion %.

- **Asset Management** (Super Admin/HR/Manager) — Dashboard/Reports tabs, KPI row (Total Assets/Assigned/In Store, counting only active/non-disposed assets). **Asset Inventory**: every asset gets an auto-generated tracking tag (`AST-0001`, …); **Edit** (name/category/cost/warranty date) is a distinct action from **Assign**, so editing never silently changes who holds an asset. **Assign** to any active employee, **Return** it to the pool, **Transfer** it directly to a different employee in one step, **Send for Repair** / **Back In Store**, **Pause/Resume** (retires it from the active pool without deleting it), or **Dispose** of it (terminal, requires it to be returned first). **Log Audit** appends a timestamped check to the asset's **History**, alongside every other action ever taken on it. **Rule: an asset cannot be assigned to more than one active employee at a time** — an already-assigned asset must be returned/transferred rather than silently reassigned (enforced server-side). Assets added by a Manager/Assistant Manager start **Pending Approval** and need Super Admin/HR Admin sign-off before they can be assigned — assets added by Super Admin/HR Admin auto-approve. **Reports** tab: totals by category and by status, total fleet value.

  All four new modules' **Key Features** tile grids are functional, jumping to (or, for Reports tiles, switching to) the section that implements each feature — not static placeholders.
