...
- **Attendance** — matches the prototype layout: scope banner, department/date filter bar + Export (CSV), KPI row (Present/Absent/Late Check-in/Missing Punch-in), and numbered widgets — **Biometric Reports** (checked-in/out totals by method: Web/Mobile/Biometric/Face), **Regularization Requests** (Approve/Reject, routed through the shared approvals queue), and **Quick Actions** (daily marking grid, Configure Policies). Employees check in/out (method selectable) and see 30-day history + a Regularize button.
- **Leave Management** — scope banner, filters + Export, KPI row (Pending/Approved MTD/Rejected MTD/On Leave Today), and widgets — **Leave Approval Chain** (derived live from the Organization Structure role workflow, e.g. "TL → STL → AM → Manager → HR Admin → Super Admin"), **Leave Types & Policy** (CL/SL/EL/Maternity/Paternity/LWP, Super-Admin-editable quotas), and **Employees on Leave — Department Wise**. Employees see their Casual/Sick/Earned balance, apply (days auto-computed, blocked over balance), and track requests; HR approve/reject deducts the balance.
- **Payroll** — scope banner, filters + Export, KPI row (Payroll Cycle/Employees Processed/F&F Requests — F&F derived from Exited employees), and a full **Salary Structure** breakdown widget (Basic/HRA/Conveyance/Special Allowance → Gross; PF/PT/TDS → Total Deductions; Net Pay) plus **Quick Actions** (Run Payroll for a period, configure structures, Configure Policies). HR edits every component per employee; running payroll generates one payslip per Active employee (idempotent per period). Employees see their own breakdown and download payslips.
