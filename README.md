...
- **Timesheet** — employees log daily hours against a project they're assigned to (date, task description, hours), pending HR approval. HR approves/rejects entries and a Reports screen totals approved hours by project and by employee. A **My Tasks** section (matching the prototype's task table + "+ New Task" modal) lists Task/Sub Task Name/Status/Start/End Date/Updates, with a Dependent-Task toggle that links a task to another it depends on. A manager/HR ("higher authority") can assign a task straight to any employee via the modal's **Assign To** picker — it then shows up on that employee's own My Tasks list, with a real notification ("X assigned you a task"), and the assignee can update its status and post threaded updates.
