...
export default function Documents() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      await api.post('/documents', { title, category, mandatory, file_data_url });
      setTitle(''); setCategory('Policy'); setMandatory(false); setFile(null); setShowForm(false); load();
        ...
        {documents.map((d) => (
          <div key={d.id} style={{ borderTop: '1px solid #EEF0F3', padding: '10px 0' }}>
            ...
            <div className="row" style={{ marginTop: 6, gap: 6 }}>
              ...
              {!isHR && d.mandatory && !d.acknowledgedByMe && <button className="primary" onClick={() => acknowledge(d.id)}>Acknowledge</button>}
              {isHR && d.mandatory && <button onClick={() => viewAck(d.id)}>Acknowledgments</button>}
              {isHR && <button onClick={() => remove(d.id)}>Delete</button>}
            </div>
          </div>
        ))}
      </div>
