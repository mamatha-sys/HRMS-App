...
import { upsertAttendanceForDate, recomputeLateFlags } from './attendanceCore.js';
...
function applyPunchToAttendance(employeeId, timestamp, punchType) {
  ...
  if (punchType === 'check-out') {
    upsertAttendanceForDate(employeeId, date, { check_out_time: time, method });
  } else {
    ...
    if (!existing?.check_in_time) upsertAttendanceForDate(employeeId, date, { status: 'Present', check_in_time: time, method });
    else if (!existing.check_out_time) upsertAttendanceForDate(employeeId, date, { check_out_time: time });
  }
